...
=== Send various body sensor data - HR, RR, SPO2, SDDN

* Intent: `com.urbandroid.sleep.ACTION_EXTRA_DATA_UPDATE`
This is a generic intent intent to carry data from various sensors.
The data can be sent as a single float value or as an array of floats.

* Extras:

** *Data type marker.* Presence of a particular marker determines what are carried.
   If only a single value is sent, it is stored directly in this extra.
   Four markers are supported at the moment:
*** com.urbandroid.sleep.EXTRA_DATA_HR - heart rate. The frequency does not need to be high, once every few minutes is fine.
*** com.urbandroid.sleep.EXTRA_DATA_RR - R-R intervals - intervals between consecutive heart beats. This data need to be reliable. Every R-R interval should be sent.
*** com.urbandroid.sleep.EXTRA_DATA_SPO2 - blood oxygenation measure. High frequency (every few seconds) is required.
*** com.urbandroid.sleep.EXTRA_DATA_SDDN - short-term heart rate variability. Typically once every few minutes.

The further extras depend on whether a single value is sent or an array.

** *Sinle value case*
**** com.urbandroid.sleep.EXTRA_DATA_TIMESTAMP - optional. The time of the measurement. Long - unix timestamp.
**** The value is stored directly in the ,arker extra as a float.

** *Array case*
**** com.urbandroid.sleep.EXTRA_DATA_TIMESTAMP - optional. The time of the last measurement. Long - unix timestamp.
**** com.urbandroid.sleep.EXTRA_DATA_FRAMERATE - optional. Long, millis. The default is 1000 (i.e. 1s)
