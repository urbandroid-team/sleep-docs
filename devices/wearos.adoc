...
Some wearables can also collect SpO2 data - for more details, please see the chapter on <</devices/oximeter#, oximeters and SpO data>>. Currently, Ticwatch Pro 3, Ticwatch E 3, Fossil Gen 6, and TicWatch Atlas have integrated SpO2 data.
...
* Wear OS 4.0 (with some limitations)
* Wear OS 5.0 (with some limitations)

WARNING: Since Wear OS 3.0, there have been some  limitations implemented for independent apps:
- not all wearables allow reading SpO2 or HRV data (Pixel Watch for example)
- Bedtime mode might NOT allow vibrations for alarm
...
MobVoi also has a scheduled *Essential mode* optimization on some Ticwatch models, that will disrupt the data flow.
OnePlus watches have a scheduled *Hibernation mode* in _Settings -> Battery_, that will disconnect the wearable.
