...
NOTE: You need to have the lullaby automatic start enabled, or start the lullaby manually from the tracking screen with the icon:ic_action_lullaby[] icon in the top right corner.
Session::
 * Meditation Daytime
 * Meditation Fall Asleep
 * Wim Hof Inspired Breathing Exercise
Voice setup:: Opens the settings for the Text-to-speech engine on your phone, where you can change the voice parameters, like tone, langauge, and rate.
