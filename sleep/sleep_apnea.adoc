...
In the morning Sleep as Android determines your <</sleep/breath_rate/rdi#,RDI>> with the related severity scoring. Score over RDI >15 per hour is considered as a significant warning sign and visiting a sleep lab is highly recommended.

The blue line shown in graph is SpO2 curve. Every significant dip in SpO2 is marked by a O2 icon:ic_action_cpap[] symbol. This indicates 1 breathing disturbance episode. Every such sign contributes 1/(sleep length hours) to the <</sleep/breath_rate/rdi#,RDI>>.
