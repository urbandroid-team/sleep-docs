...
The next day, either a pop-up notification with estimated timings appears waiting for your confirmation (Manual only mode), or the entry is automatically saved (Save automatically mode).
...
Manual only:: A notification will be displayed asking for confirmation. Sleep time estimation is saved only if you confirm it.
