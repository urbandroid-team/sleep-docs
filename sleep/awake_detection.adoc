...
=== Movement intensity
Movement intensity awake detection ss based on *significant activity* measured by a motion detection sensor e.g. Accelerometer in phone or on a wearable, Sonar, Sleep phaser... see <</sleep/sensors#, Sensors>>
[horizontal]
...
When sing a heart rate sensor, you give Sleep very precise information about your awake times. During sleep, your heart beats approximately  up to 10 BPM slower than when awake.
...
=== Light
*Ambient light* in your bedroom after sunset is another hint of you being awake. We use the sensor on the phone for light tracking. If there is disturbing light after sunset (value depends on set sensitivity), we consider it as an Awake.
...
=== Talk
Several sounds, like talking, baby cry, etc. (see <</sleep/sound_recognition#, Sound recognition>>), may indicate you are awake. Because every mic is different, and every environment has different acoustics, it would be impossible to balance all these specifics automatically.
...
* *Higher sensitivity* means that also sounds where we are less sure about their classification (e.g. baby cry) should mark an awake period.
...
This option adds awake automatically when the phone is held in up right position, there is minimum hand shaking and screen os on.

NOTE: With all settings we require the screen to be on for longer than your _Phone settings -> Display -> Screen timeout_. This is because your phone's screen may turn on by other unconscious event such as the phone is fully charges etc...
