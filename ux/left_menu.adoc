...
| icon:ic_alarm[] | *Alarms*: Opens <</homecreen#,List of alarms>> in Dashboard mode.
| icon:ic_timelapse[] | *Stats*: Opens <</sleep/charts#,Statistics section>>.
| icon:ic_action_graph[] | *Graphs*: Opens <</sleep/sleep_graph#,Graph section>> in Dashboard mode.
| icon:ic_action_stats[] | *Charts*: Opens <</sleep/charts#,Charts section>>.
...
| icon:trophy[] | *Achievements*: A shortcut to <</sleep/achievements#, Achievement board>>.
| icon:ic_droid[] | *Sleep Assistant*: Opens <</sleep/assistant#,Sleep Assistant>>.
|===
...
|===
| Icon | Description
| icon:ic_cloud_upload[] | *Shortcuts*: Opens Shortcut section. For more details on Shortcuts, see <</services/backup_data#,Backup>>.
| icon:ic_action_lullaby[] | *Lullabies*: A shortcut to <</sleep/lullaby#, lullaby section>>.
...
| icon:ic_dashboard[] | *Dashboard*: Presents all options in cards, see <</ux/hs_dashboard#, details about Dashboard>>.
| icon:ic_tab[] | *Tabs*: Presents all option in tabs, list of alarm is the main screen, see <</ux/hr_tabs#,details about Tabs>>.
| icon:ic_action_track_off_white[] | *Alarms only*: Simplified mode without any tracking-related settings, read more in <</ux/hs_alarms_only#,Alarms only mode>> chapter.
|===
...
| icon:ic_cloud_upload[] | *Backup*: Opens back up dialogue. For more details on Back up, see <</services/backup_data#,Backup>>.
| icon:ic_incognito[] | *Privacy*: Opens the <</general/privacy#, Privacy section>>
