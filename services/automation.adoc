...
NOTE: All automation setups are DIY and you need to make sure the client is properly configured.
...
alarm_snooze_canceled:: You have canceled an alarm that is currently snoozed.
We are sending the following values:
* value1: UNIX timestamp of the alarm start time, example: "1582719660934"
* value2: alarm label, example: "label" (Any tabs and newline characters in the label will be removed before sending)
...
* value1: UNIX timestamp of the alarm start time, example: "1582719660934"
* value2: alarm label, example: "label" (Any tabs and newline characters in the label will be removed before sending)
alarm_rescheduled:: Fires when the app is saving a new alarm time, different from the previous alarm time  (it allows an external automation system to track the latest set alarm time on Sleep as Android).
* value1: UNIX timestamp of the alarm start time, example: "1582719660934"
* value2: alarm label, example: "label" (Any tabs and newline characters in the label will be removed before sending)
...
* value1: UNIX timestamp of the alarm start time, example: "1582719660934"
* value2: alarm label, example: "label" (Any tabs and newline characters in the label will be removed before sending)
before_alarm:: Fires exactly 1 hour before the next alarm is triggered.
* value1: UNIX timestamp of the alarm start time, example: "1582719660934"
...
alarm_wake_up_check:: Fires when the wake-up check notification is triggered.
alarm_rescheduled:: Fires when the the app is saving the next alarm time (after snoozing, after alarm start, after alarm dismiss).
jet_lag_start:: Fires when the JetLag prevention feature starts.
jet_lag_stop:: Fires when the JetLag prevention feature is finished.
